...
| SESSION-WORK-005 | Same-session current-claim reservation | A claim invocation snapshots the session's current-claim pointer once, adopts a live pointed-to assignment before considering fresh work, and replaces a stale pointer only through value-CAS. Competing same-session claims have one pointer winner; a lost claimant releases only its minted work and emits no downstream work result. An undelivered-result unwind clears only when its expected pointer still matches, preserving a later winner. | `internal/session/store_test.go` (`TestCurrentClaimCASHasOneWinnerAndConditionalClear`); `cmd/gc/current_claim_cas_test.go` (`TestTwoHooksCASReserveOneWinnerAndReleaseOnlyLoser`, `TestCurrentClaimCASEpipeClearCannotEraseLaterWinner`); `cmd/gc/cmd_hook_claim.go` |
