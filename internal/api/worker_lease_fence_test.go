...
func heartbeatBody(t *testing.T, state State, beadID string) string {
	t.Helper()
...
func TestWorkerHeartbeatCannotStampOverANewerHolder(t *testing.T) {
	...
	if rec := postWorker(t, h, state, "/worker/claim", heartbeatBody(t, state, created.ID)); rec.Code != http.StatusOK {
	...
	rec := postWorker(t, h, state, "/worker/heartbeat", heartbeatBody(t, state, created.ID))
...
func TestWorkerClaimCannotStampLeaseOverANewerHolder(t *testing.T) {
	...
	rec := postWorker(t, h, state, "/worker/claim", heartbeatBody(t, state, created.ID))
	if rec.Code != http.StatusConflict {
