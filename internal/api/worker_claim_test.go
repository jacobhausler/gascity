...
func seedWorkerSession(t *testing.T, store beads.Store) beads.Bead {
	t.Helper()
	sess, err := store.Create(beads.Bead{
		Title:  "worker test session",
		Type:   session.BeadType,
		Labels: []string{session.LabelSession},
	})
	if err != nil {
		t.Fatalf("create worker test session: %v", err)
	}
	return sess
}

func workerSessionID(t *testing.T, state State) string {
	t.Helper()
	store := state.SessionsBeadStore().Store
	if store == nil {
		t.Fatal("worker test state has no session store")
	}
	sessions, err := store.ListByLabel(session.LabelSession, 0)
	if err != nil {
		t.Fatalf("list worker test sessions: %v", err)
	}
	if len(sessions) == 0 {
		return seedWorkerSession(t, store).ID
	}
	return sessions[0].ID
}

func workerClaimBody(t *testing.T, state State, beadID string) string {
	t.Helper()
	return `{"session_id":"` + workerSessionID(t, state) + `","assignee":"worker-local-3-pool","bead_id":"` + beadID + `"}`
}
...
// claimingHiddenStoreDraft keeps the native store's conditional writer while
// hiding Claim, so the route's capability refusal is tested against a store
// that genuinely does not expose the worker-acquire seam.
type claimingHiddenStoreDraft struct {
	beads.Store
}
...
func TestWorkerClaimSetsAssigneeAndReturnsTheBeadWhole(t *testing.T) {
	...
	body := workerClaimBody(t, state, created.ID)
...
func TestWorkerClaimIsSingleWinner(t *testing.T) {
	...
	state.cityBeadStore = rigStore
	h := newTestCityHandler(t, state)
	firstSession := workerSessionID(t, state)
	secondSession := seedWorkerSession(t, rigStore).ID

	claim := func(assignee string) *httptest.ResponseRecorder {
		sessionID := firstSession
		if assignee != "worker-local-3-pool" {
			sessionID = secondSession
		}
		body := `{"session_id":"` + sessionID + `","assignee":"` + assignee +
			`","bead_id":"` + created.ID + `"}`
		req := newPostRequest(cityURL(state, "/worker/claim"), strings.NewReader(body))
		rec := httptest.NewRecorder()
		h.ServeHTTP(rec, req)
...
func TestWorkerClaimRefusesInsteadOfEmulatingWhenStoreLacksTheCapability(t *testing.T) {
	plainStore := beads.NewMemStore()
	created, err := plainStore.Create(beads.Bead{Title: "unsupported", Status: "open"})
	if err != nil {
		t.Fatalf("Create: %v", err)
	}
	state := newFakeState(t)
	hidden := claimingHiddenStoreDraft{Store: plainStore}
	state.stores = map[string]beads.Store{"myrig": hidden}
	state.cityBeadStore = hidden
	h := newTestCityHandler(t, state)
