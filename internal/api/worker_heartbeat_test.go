...
func workerClaim(t *testing.T, h http.Handler, state State, beadID string) {
	t.Helper()
	body := workerClaimBody(t, state, beadID)
...
func TestWorkerHeartbeatRejectedForNonHolder(t *testing.T) {
	...
	h := newTestCityHandler(t, state)
	workerClaim(t, h, state, created.ID)
	otherSession := seedWorkerSession(t, rigStore).ID
	...
	rec := workerHeartbeat(t, h, state, created.ID, otherSession, "worker-local-4-pool")
