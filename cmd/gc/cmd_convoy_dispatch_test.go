...
// #5584 across the cmd/gc boundary: the dynamic-fragment fallback route is
// assembled by graphFallbackBindingForBead, which knows nothing about agent
// lifecycles, so the one-shot rule has to be applied by the router in
// internal/graphroute rather than at every caller.
...
// A per-step gc.run_target naming a one-shot pool is independent even when the
// workflow default is routed to a long-lived agent.
