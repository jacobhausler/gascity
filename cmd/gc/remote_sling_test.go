...
func (f *remoteSlingFake) serve(w http.ResponseWriter, r *http.Request) {
	...
	case strings.HasPrefix(p, "/v0/city/mc/beads/"):
		id := strings.TrimPrefix(p, "/v0/city/mc/beads/")
		body, ok := f.beads[id]
		if !ok {
...
const remoteSlingRigs = `[{"name":"west","path":"/city/west","prefix":"ws","agent_count":1,"running_count":0,"suspended":false}]`
...
func TestCmdSlingRemoteDryRun_ReportsIdempotentNoOp(t *testing.T) {
	...
	for _, want := range []string{"already routed to mayor", "nothing (idempotent no-op)"} {
		if !strings.Contains(out.String(), want) {
			t.Errorf("preview missing %q: %q", want, out.String())
		}
	}
	for _, call := range fake.callsFor() {
		if !strings.HasPrefix(call, "GET ") {
