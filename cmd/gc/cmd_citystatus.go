...
type StoreHealth struct {
	...
	// LiveRowsComplete is false when LiveRows is the bounded-scan fallback
	// zero rather than a real measurement (the count did not finish within
	// statusStoreHealthTimeout) — see storeHealthFromInputs (ra-nka1c).
	LiveRowsComplete bool    `json:"live_rows_complete"`
...
func snapshotFromStatusView(cityPath string, v api.StatusView) cityStatusSnapshot {
	...
	if v.StoreHealth != nil {
		snapshot.Summary.StoreHealth = &StoreHealth{
			...
			// The API path (internal/api/store_health.go) never fabricates a
			// zero: countBeadStoreRows returns an error on failure/timeout
			// and the whole StoreHealth block is omitted, so a StoreHealth
			// reaching here from the API always carries a real count.
			LiveRowsComplete: true,
