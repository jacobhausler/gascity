...
// TestReconcileSessionBeads_DrainAckOwnDrainStepClosesWithoutEvent is ra-p37yo
// Arm 2: a session whose ONLY assigned work is its own mol-do-work "drain" step
...
// Before the ra-18okp close-gate fix, the drain step counted as assigned work,
// so the bead stayed open forever and the pool controller respawned a fresh
// session onto the same still-open step every ~20s (ra-18okp).
