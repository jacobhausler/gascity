...
func TestRemoteBdRoutesTheWorkerSubset(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"close", []string{"close", "mc-7"}, "POST /v0/city/mc/worker/close"},
...
func TestRemoteBdRefusesUnknownFlagsOnEveryVerb(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"close", []string{"close", "mc-7", "--reason", "done"}, "POST /v0/city/mc/worker/close"},
		{"close before the id", []string{"close", "--reason=done", "mc-7"}, "POST /v0/city/mc/worker/close"},
